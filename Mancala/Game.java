...
public class Game {
	...
	public static boolean playTurn ( int index, int stone ) {
		...
		if ( Game.checkIndex( index )  && stone > 0 ) { // make sure pit has more than 0 stones
			...
			if ( checkForWinner() ) {
				 ...
				 int playAgain = JOptionPane.showConfirmDialog(frame, "Play again?");
	...
	public static boolean checkForWinner() {
		...
		for (int i = 0, emptyPits = 0; i < 12; ++i ) {
			...
			if ( emptyPits == 6 ) {
				System.out.println("empty pits = " + emptyPits);
	...
	public static void collectEndGameBonus( int bonusSideIndexStart ) {
		System.out.println("bonusSideIndexStart is = " + bonusSideIndexStart);
		
		for (int i = bonusSideIndexStart; i < ( bonusSideIndexStart + 6 ); i++ ) {
